...
async function setData(instanceId: string, data: CallbackData): Promise<void> {
  try {
    await kv.set(`gpu:${instanceId}`, data, { ex: 3600 }); // 1 hour TTL
  } catch (e) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // POST: GPU instance reports its tunnel URL or status
  if (req.method === 'POST') {
    ...
    if (tunnel_url) {
      ...
      data.ready = true;
      console.log(`[callback] Storing tunnel URL for ${instance_id}: ${tunnel_url}`);
    }
    ...
    if (status) {
      ...
      console.log(`[callback] Status update for ${instance_id}: ${status} - ${message}`);
    }
